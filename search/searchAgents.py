...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # self.corners: corners to be explored
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            x, y = state[0]
            newX, newY = int(x + deltaX), int(y + deltaY)
            newPos = (newX, newY)
            if not self.walls[newX][newY]:
                newCorners = list(state[1])
                if(newX, newY) in newCorners:
                    newCorners.remove(newPos)
                # newCorners = tuple(x for x in state[1] if x != (newX, newY))
                newSuccessor = ((newPos, tuple(newCorners)), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    # nextCorner = 0
    # for corner in self.corners:
    #     manhDis = util.manhattanDistance(currnetPos, corner)
    #     if manhDis < min:
    #         min = manhDis
    #         nextCorner = corner
    ...
    for corner in state[1]:
        manDisList.append(util.manhattanDistance(corner, currentPos))
    maxManDis = max(manDisList)
...
def foodHeuristic(state, problem):
    ...
    for food in foodList:
        mazeDis.append(mazeDistance(position, food, startingState))
